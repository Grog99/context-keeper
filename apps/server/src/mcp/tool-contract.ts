...
export const SEARCH_MEMORY_DESCRIPTION = `Search the shared project memory (facts and documents) using full-text search.

Scope: your project's memories plus "global" memories (shared across all projects). You cannot see other projects' memories.

Returns headers only, ranked by relevance — call get_memory(id) to fetch the full body of anything that looks relevant. Default kind filter is fact + document; pass \`kind\` to narrow to just one. \`tags\` filters to memories sharing at least one of the given tags (any-of match).
