 ...
 * `proposals.errors.ts`; `ToolError` — kontrakt w `common/errors.ts`) na HTTP dla powierzchni
 * dashboardu (§M1 planu). `ProposalError('stale')` niesie dodatkowo `staleIds` w kopercie — SPA
 * używa ich do podświetlenia konfliktu (`ProposalActions` disabled + alert, §8.2 design-systemu).
 */
@Catch(ProposalError, ToolError)
export class DashboardErrorFilter implements ExceptionFilter {
  catch(exception: ProposalError | ToolError, host: ArgumentsHost): void {
