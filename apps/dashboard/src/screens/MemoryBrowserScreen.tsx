...
export function MemoryBrowserScreen() {
  ...
  const editFormRef = useRef<MemoryEditFormHandle>(null);
  // Zapis reaktywności `eventTimeInvalid` z `MemoryEditForm` (dziecko) do przyklejonego przycisku
  // "Zapisz" w rodzicu — sam ref nie wystarczy (odczyt refa nie triggeruje re-rendera rodzica przy
  // każdym wpisywanym znaku), więc dziecko zgłasza zmiany przez `onValidityChange`.
  ...
  return (
    // Bez `gridTemplateRows: 'minmax(0, 1fr)'` jedyny (auto) wiersz grida rośnie do wysokości
    // treści kolumn i przecieka poza `h-full min-h-0` — `main` (AppShell) zaczyna wtedy scrollować
    // całą stronę zamiast tego robić `flex-1 overflow-y-auto` niżej, co grzebie przyklejony pasek
    // akcji na dole. Nie "czyść" tego jako zbędnego stylu inline.
    <div
      className="grid h-full min-h-0"
      style={{ gridTemplateColumns: 'minmax(320px, 38%) 1fr', gridTemplateRows: 'minmax(0, 1fr)' }}
    >
      <div className="flex min-h-0 min-w-0 flex-col border-r border-border-strong">
      ...
      <div className="flex min-h-0 min-w-0 flex-col bg-surface">
        {!selectedId ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto px-6 py-5">
              <div
                className={cn(
                  ...
                  // Tryb edycji potrzebuje realnego łańcucha flex/height aż do textarea (Defekt 2) —
                  // reszta widoku (podgląd) celowo zostaje bez wymuszonej wysokości, żeby długi markdown
                  // dalej po prostu rozpychał tę kolumnę i scrollował się w rodzicu (S1).
                  editing && 'flex h-full min-h-0 flex-col',
                )}
              >
                ...
                {editing ? (
                  // Defekt 1 (S5, Codex): zakładki nie mogą w ogóle wyrenderować się w trybie edycji —
                  // Radix `TabsContent` domyślnie odmontowuje nieaktywną zawartość, więc kliknięcie
                  // "Metadane"/"Rewizje"/"Relacje" w trakcie edycji odmontowałoby `MemoryEditForm`,
                  // zerując `editFormRef` i wyciszając przycisk "Zapisz". Zamiast łatać to disablowaniem
                  // triggerów, edit-mode w ogóle nie montuje `Tabs`/`TabsList` — więc nie ma czego kliknąć.
                  // Ten sam branch niesie łańcuch flex/height dla Defektu 2 (S5): `flex-1 min-h-0`, żeby
                  // `MemoryEditForm` dostał realną, resolvowalną wysokość aż do textarea.
                  ...
                  <Tabs value={tab} onValueChange={setTab}>
                    ...
                    <TabsContent value="body" className="pt-4">
                      <div
                        className={cn(
                          'rounded-md border border-border bg-muted/40 px-4 py-3 text-md leading-relaxed text-foreground [&_code]:rounded [&_code]:bg-muted [&_code]:px-1 [&_code]:py-0.5 [&_code]:font-mono [&_code]:text-sm [&_pre]:overflow-x-auto [&_pre]:rounded-md [&_pre]:bg-muted [&_pre]:p-3',
                          detail.kind === 'document' ? 'max-w-[72ch]' : 'max-w-[66ch]',
                        )}
                      >
            ...
            <div className="flex flex-none items-center border-t border-border px-6 py-3">
              <div className="mx-auto flex w-full max-w-4xl items-center gap-2">
                {editing ? (
                  <>
                    <Button
                      variant="secondary"
                      onClick={() => setEditingForId(null)}
                    ...
                    <Button
                      ...
                      disabled={editMutation.isPending || !editValid}
                    >
...
type MemoryEditFormHandle = { submit: () => void };

const MemoryEditForm = forwardRef<
  MemoryEditFormHandle,
  {
    kind: MemoryKind;
...
>(function MemoryEditForm(
  { kind, initialHeader, initialBody, initialTags, initialEventTime, onSave, saving, onValidityChange },
  ref,
) {
  ...
  // Przyklejony przycisk "Zapisz" żyje w rodzicu (MemoryBrowserScreen), więc walidacja musi
  // wypłynąć z dziecka na zewnątrz — sam `submit()` przez ref nie wystarczy, bo rodzic nie
  // wie, kiedy stać się disabled, dopóki użytkownik faktycznie nie kliknie.
  useEffect(() => {
    onValidityChange(!eventTimeInvalid);
  }, [eventTimeInvalid, onValidityChange]);

  useImperativeHandle(ref, () => ({
    submit: () => {
      if (saving || eventTimeInvalid) return;
      onSave({
        header,
        body,
        tags: tagsInput
