> **Archiwum — snapshot roadmapy z 2026-07-29 (v1.3 domknięte).**
> Zamrożony po ukończeniu całego zakresu v1.3 (dostęp + agent/MCP + UI), przed otwarciem v1.4
> — pełne opisy pozycji v1.3 zostają tutaj, żywa roadmapa niesie tylko jednozdaniowe podsumowania
> + aktualny zakres. Snapshot niesie też pracę off-plan wykrytą przy reconcile, której poprzednia
> wersja żywej roadmapy nie miała. Żywa roadmapa: [`../roadmap.md`](../roadmap.md).
> Poprzedni snapshot: [`roadmap-2026-07-27-v1.2-complete.md`](roadmap-2026-07-27-v1.2-complete.md).
...
**Aktualizacja:** 2026-07-29 · **Etap:** v1.3 domknięte (dostęp i UI) → wchodzimy w **v1.4**.

Legenda: ✅ zrobione · 🔨 w toku · ⬜ przed nami

> Pełne opisy zakresu faz 0 → v1.2 zarchiwizowane w
> [`roadmap-2026-07-27-v1.2-complete.md`](roadmap-2026-07-27-v1.2-complete.md).
> Wcześniejszy snapshot (v1 domknięte): [`roadmap-2026-07-23-v1-complete.md`](roadmap-2026-07-23-v1-complete.md).
...
- **Czytelność przeglądarki pamięci** ✅ — panel szczegółów zajmuje teraz całą dostępną wysokość
  ...
  czytania treści zależy od `kind` (`document` 72ch, `fact`/`event` 66ch). Domknięte trzema fixami
  poza pierwotnym opisem: pojedyncze złamania linii w treści są zachowywane (dotąd markdown je zjadał),
  `AppShell` mieści się w viewporcie przy bardzo niskich oknach, a porzucenie edycji z niezapisanymi
  zmianami wymaga potwierdzenia.
...
### Poza planem (dopisane przy reconcile 2026-07-29)

- **Rewizja kontrastu + kolor tożsamości dla `kind`** ✅ — przejście po całym dashboardzie pod kątem
  kontrastu wg design-systemu plus spójny kolor tożsamości dla `fact`/`document`/`event`, żeby rodzaj
  wpisu dało się rozpoznać bez czytania etykiety. Nie było pozycją w roadmapie — wyszło z dogfoodingu
  w trakcie fazy UI.
- **Fallback ścieżek `.env` i migracji dla `pnpm dev`** ✅ — serwer odpalany przez `pnpm dev` szukał
  `.env` i katalogu migracji względem innego cwd niż build produkcyjny; fix DX, bez wpływu na runtime
  produkcyjny.
- **Pierwszy przegląd techniczny** ✅ — [`tech-review.md`](../tech-review.md): fan-out trzech
  recenzentów read-only (dryf dokumentacji · struktura/zależności · testy/dane/operacje) po całej
  historii repo, 15 ustaleń z dowodami `plik:linia`, każde zweryfikowane w źródle. Przyjęte pozycje
  mają lustro w [`backlog.md`](../backlog.md) → „Dług techniczny / architektura"; `tech-stack.md`
  zsynchronizowany z kodem (§4, §5, §6, §12, §13).
