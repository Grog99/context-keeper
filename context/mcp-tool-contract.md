...
```
Propose a new fact to add to the shared project memory.
...
One atomic fact per call. Do not bundle multiple unrelated facts into a single header/body —
call save_memory once per fact.
...
You can only CREATE new facts in v1 (no update/delete). To correct an existing fact, save a new
one that references what it supersedes in the body — a human reviewer will reconcile the two.

NEVER include secrets (API keys, passwords, private keys, tokens, credentials) in header or body.
Such content is rejected before it reaches storage (`secret_blocked` error) — the fact is not
saved, and there is no in-place redaction to fall back on. Rewrite the fact referring to the
secret by name or purpose only, never by value, and try again.
...
- status "duplicate_pending": an identical proposal is already pending — `id` refers to that
  existing proposal, not a new one.
- status "already_exists": an identical fact is already approved in memory — `id` refers to that
  memory.
